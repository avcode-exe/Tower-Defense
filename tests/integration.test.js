...
describe('integration', () => {
  let makeGame, swordsmanSpec, archerSpec, healerSpec, lightningSpec, placeMonsterAt, makeTroop;
  ...
  it('melee troop attacks monster via step', () => {
    const game = makeGame();
    game.placeTroop(swordsmanSpec, 5, 5);
    const m = placeMonsterAt(game, 1, 5, 5);
    game._updateMonsterTileIndex();
    // Use the game's stepTroops which handles mock setup
    game._stepTroops(1);
    // Monster should exist and have taken damage or still be alive
    expect(game.monsters.length).toBeGreaterThan(0);
    expect(m.hp <= m.maxHp).toBe(true);
  });

  it('monster leaking reduces lives', () => {
    const game = makeGame();
    game.spawnMonster(1);
    const m = game.monsters[0];
    m.distance = m.totalLength + 100;
    game.state = 'WAVE_ACTIVE';
    game._stepMonsters(1);
    expect(game.lives).toBeLessThan(25);
  });

  it('sell/refund cycle', () => {
    const game = makeGame({ gold: 5000 });
    game.placeTroop(swordsmanSpec, 1, 1);
    game.sellTroop(0);
    expect(game.gold).toBeGreaterThan(4000);
  });

  it('upgrade increases damage', () => {
    const game = makeGame({ gold: 5000 });
    game.placeTroop(swordsmanSpec, 1, 1);
    const dmgBefore = game.troops[0]._cachedDamage;
    game.upgradeTroopStat(0, 'dmg');
    expect(game.troops[0]._cachedDamage).toBeGreaterThan(dmgBefore);
  });

  it('dev mode gold is Infinity', () => {
